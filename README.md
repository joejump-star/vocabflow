...
- **3D 語塊筆記**：不只記「單字＝中文」。查一個字會拆出依語法槽位分組的**常用搭配**（`cynical + 名詞`、`副詞 + cynical`）；也能直接查**語塊**（`have a bearing on`），看它的模組化變化階梯與造句模板。語塊依王梓沅「3D 英文筆記術」的**八大公式**分類（片語／片語動詞／文法搭配／語彙搭配／慣用句頭／慣用句中／情境式語塊／句子框架）
- **畢業機制**：記熟的字可以「🎓 我記熟了」移出字庫，釋放空間並計入已精通
...
- **備份與還原**：匯出成 JSON 交給系統分享表單（存 iCloud、傳給自己都行），或直接複製文字保存
...
反過來說：**清除瀏覽器網站資料就會失去所有單字**，請善用 Word Bank 頁右上角 ⚙ 的匯出備份。備份檔**不包含** API key——金鑰是機密，不會跟著檔案流出去。
...
- **字庫容量受瀏覽器限制**：資料存在 localStorage，iOS Safari 約 5MB。一個「完整規格」的字約 2.6KB，估計上限 1,500–1,800 字。接近上限時 app 會先自動清掉可重建的查詢快取；真的滿了會跳紅色警示要求備份，不會默默掉進度

## 開發路線

- [ ] 造句練功房：使用者用模板寫自己的句子，Gemini 批改是否自然
- [ ] 搭配詞挖空練習題（`a cynical ___`）
- [ ] 聽力模式：只播發音，選中文意思
- [ ] 連續學習天數 streak

## 想改造這個專案？

看 [DEVELOPMENT.md](DEVELOPMENT.md)——資料模型、儲存容量上限、Gemini prompt 設計、安全性規範都在裡面。
